...
**models/** - Обученные модели (не включаются в Git)  
│  
...
**results/** - Результаты экспериментов (не включаются в Git)  
│  
...
└── `venv_autokeras/` - Папка виртуального окружения (не в Git)
...
AutoKeras (60 trials) vs две ручные архитектуры нейросетей
Каждая архитектура обучается ~ 1 час
